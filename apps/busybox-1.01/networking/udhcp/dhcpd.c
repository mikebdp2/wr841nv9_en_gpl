...
#endif
{
	...
	/*  by huangwenzhong, 29Mar13 */
#if MODULE_SMART_DMZ
	tp_static_lease *tp_lease = NULL;
	uint8_t *read_data = xmalloc(sizeof(tp_static_lease));
	int read_len = 0;
	int sig = 0;
#endif
...
/*  by huangwenzhong, 29Mar13 */
#if MODULE_SMART_DMZ
		max_sock = udhcps_sp_fd_set(&rfds, server_socket);
#else
		...
		/*  by huangwenzhong, 29Mar13 */
#if MODULE_SMART_DMZ
		read_len = udhcps_sp_read(&rfds, read_data, sizeof(tp_static_lease));
		sig = 0;
		
		if (read_len > sizeof(int))
		{
			tp_lease = (tp_static_lease*)read_data;
			tp_handle_static_lease(&(server_config.static_leases), 
											(tp_static_lease*)read_data);
			sig = 0;
			continue;
		}
		else if (read_len > 0)
		{			
			memcpy(&sig, read_data, sizeof(int));
		}
