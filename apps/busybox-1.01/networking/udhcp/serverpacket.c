...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	struct static_option *lease_option  = NULL;
...
#if MODULE_SMART_DMZ/*  by huangwenzhong, 08Sep12 */
		lease_option = getOptionByMac(server_config.static_leases, oldpacket->chaddr);
		if (NULL != lease_option)
		{
			lease_time_align = getLeaseTimeByMac(server_config.static_leases, oldpacket->chaddr);
			lease_option = NULL;
		}
#endif
...
#if MODULE_SMART_DMZ/*  by huangwenzhong, 29Mar13 */
	if (lease_option != NULL)
	{
		tmp_option = lease_option;
		while(tmp_option != NULL)
		{
			find_and_replace_option(packet.options, tmp_option->option, tmp_option->len, tmp_option->data);
...
#if MODULE_SMART_DMZ/*  by huangwenzhong, 29Mar13 */
	struct static_option *lease_option  = NULL;/*  by huangwenzhong, 22Aug12 */
	struct static_option *tmp_option = NULL;
#endif
...
#if MODULE_SMART_DMZ/*  by huangwenzhong, 29Mar13 */
	if (lease_option != NULL)/* replace some options for twin ip pc */
	{
		tmp_option = lease_option;
		while(tmp_option != NULL)
		{
