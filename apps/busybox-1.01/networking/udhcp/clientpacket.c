...
/*
 * added by tiger, for DHCP unicast or broadcast mode setting
 */
static dhcp_flags_t bootp_flags = 0; 
...
{
    if (DHCP_FLAGS_UNICAST == bootp_flags)
        bootp_flags = DHCP_FLAGS_BROADCAST;
    else
        bootp_flags = DHCP_FLAGS_UNICAST;
}

/*  by huangwenzhong, 22Nov12 */
/* 
if support TR069,we can send "dslforum.org" to 
tell DHCP Server that we can get ACS URL from dhcp
option 43
*/
#ifdef MODULE_TR069
#define DHCP_VENDOR_ID_STR	"beelinerouter"
#else
#define DHCP_VENDOR_ID_STR	"MSFT 5.0"
#endif
...
static void init_packet(struct dhcpMessage *packet, char type)
{
	struct vendor  {
		char vendor, length;
		char str[sizeof(DHCP_VENDOR_ID_STR)];
	} vendor_id = { DHCP_VENDOR,  sizeof(DHCP_VENDOR_ID_STR) - 1, DHCP_VENDOR_ID_STR};
	...
	if (DHCP_FLAGS_UNICAST == get_runtime_dhcp_flags())			/* server reply mode choose */
	...
	if (client_config.clientid)
	    add_option_string(packet->options, client_config.clientid);/* client id */
	if (client_config.hostname) 
		add_option_string(packet->options, client_config.hostname);/* hostname */
