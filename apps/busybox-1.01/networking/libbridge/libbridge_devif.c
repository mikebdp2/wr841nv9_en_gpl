...
int br_set_guestnetwork_vap(struct bridge *br, int ifindex)
{
	if (br_device_ioctl(br, BRCTL_SET_GUESTNETWORK_VAP, ifindex, 0, 0)<0)
		return errno;

	return 0;
}

int br_set_guestnetwork_state(struct bridge *br, int value)
{
	if (br_device_ioctl(br, BRCTL_SET_GUESTNETWORK_STATE, value, 0, 0) <0)
		return errno;

	return 0;
}

int br_set_guestnetwork_mark(struct bridge *br, unsigned long value)
{
	if (br_device_ioctl(br, BRCTL_SET_GUESTNETWORK_MARK, value, 0, 0) <0)
		return errno;
