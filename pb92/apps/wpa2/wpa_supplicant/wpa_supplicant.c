			      ...
			      struct wpa_ssid *ssid)
{
	...
	system("echo ---wpa_supplicant_associate---debug by HouXB>/dev/ttyS0");
	wpa_s->reassociate = 0;
	if (bss) {
...
{
	struct wpa_supplicant *wpa_s = ctx;
	if((wpa_s->wpa_state == WPA_ASSOCIATED) || (wpa_s->wpa_state == WPA_4WAY_HANDSHAKE)
		||  (wpa_s->wpa_state == WPA_GROUP_HANDSHAKE) || (wpa_s->wpa_state == WPA_COMPLETED))
	{

    }
	else
	{
		system("echo -----eapol state error debug by HouXB---->/dev/ttyS0");
		return;
	}	
