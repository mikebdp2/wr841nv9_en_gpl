...
void wpa_debug_print_timestamp(void)
{
	...
	printf("%ld.%06u: ", (long) tv.sec, (unsigned int) tv.usec);
        #else   /* Atheros */
        {
            /* Keep the timestamp small. We usually don't care about
            *   the absolute time, we're only looking for differences;
            *   wrapping every 1000 seconds is reasonable.
            */
            int sec = tv.sec % 1000;
            int msec = tv.usec / 1000;
...
void WPA_DEBUG_PRINT(const char *location, char *cmd, ...)
{
	int len = MAX_BUF_SIZE - strlen(location) - 128/*safe space*/;
	...
	char cmdbuf[MAX_BUF_SIZE] = {0};
	sprintf(cmdbuf, WPA_DEBUG_FORMAT, location, tmpbuf);
