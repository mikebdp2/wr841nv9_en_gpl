		...
		const u8 *key, size_t key_len, int txkey)
{
	...
	{
		/* debug by yangnaijun, 20Dec11 */
		/* When change to no crypt, set drop_encry bit 0;
		 * Do nothing, if it is wpa or wpa2
		 */
		if (!hapd->conf->wpa)
		{
		wext_set_key(priv, WPA_ALG_NONE, addr, 
						key_idx, txkey, NULL, 0, key, key_len);
		}
		/* end debug */
