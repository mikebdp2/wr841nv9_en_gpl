...
#ifdef CONFIG_TP_TWIN_IP_MODULE/*  by huangwenzhong, 29Mar13 */
...
#ifdef CONFIG_TP_TWIN_IP_MODULE/*  by huangwenzhong, 29Mar13 */
	bSendArp = tp_twin_ip_is_send_arp(sha, tip);
	if (bSendArp > 0)
	{
		if (bSendArp == 1)
		{
				   ...
				   struct neighbour *n)
{
	...
	seq_printf(seq, "%-16s 0x%-10x0x%-10x%s     *        %s\n", tbuf, hatype,
		arp_state_to_flags(n), mac2asc(hbuffer,n->ha,dev->addr_len), dev->name);
	read_unlock(&n->lock);
}
