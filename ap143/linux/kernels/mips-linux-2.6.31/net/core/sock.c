...
#define MAX_BACKLOG_COUNT		(2 * 1024)

int sk_backlog_limited(struct sock *sk)
{
	struct sk_buff *skb = sk->sk_backlog.head;
	int count = 0;
	struct inet_sock *inet = inet_sk(sk);

	if ((ntohs(inet->sport) != DHCP_SERVER_PORT) 
		&& (ntohs(inet->sport) != UPNP_SERVER_PORT))
	{
		return 0;
	}
	...
	return (count < MAX_BACKLOG_COUNT)? 0 : 1;
}
EXPORT_SYMBOL(sk_backlog_limited);
